...
export const CHART = ["#9b7bf7", "#b69aff", "#7c5cff", "#c4b5fd", "#86efac", "#5fb6ff"];
...
export function PageHeader({
    ...
    return (
        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 1.5, mb: 2.5 }}>
            <Box>
                <Typography sx={{ fontWeight: 800, fontSize: "1.4rem", color: C.text, letterSpacing: "-0.02em" }}>{title}</Typography>
                {subtitle && <Typography sx={{ color: C.textMuted, fontSize: "0.82rem", mt: 0.25 }}>{subtitle}</Typography>}
            </Box>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                <Chip
                    label="Last 24 hours"
                    size="small"
                    sx={{ height: 28, fontSize: "0.76rem", color: C.accentLight, bgcolor: C.accentDim, border: `1px solid ${C.accent}44`, borderRadius: "8px" }}
                />
                {action}
            </Box>
        </Box>
...
export function Panel({
    ...
    return (
        <Card sx={{ ...glass, height: "100%", boxShadow: "0 8px 32px rgba(0,0,0,0.30)" }}>
            {title && (
                <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", px: 1.75, py: 1.25, borderBottom: `1px solid ${C.border}` }}>
                    <Typography sx={{ fontWeight: 600, color: C.text, fontSize: "0.85rem" }}>{title}</Typography>
...
export function Sparkline({ values, color = C.accent, height = 34 }: { values: number[]; color?: string; height?: number }) {
...
export function KpiTile({
    ...
    const up = (delta ?? 0) >= 0;
    const tile = (
        <Card sx={{ ...glass, p: 1.5, height: "100%", transition: "border-color 0.15s", "&:hover": { borderColor: href ? C.accent : C.border } }}>
            <Typography sx={{ color: C.textMuted, fontSize: "0.7rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em" }}>{label}</Typography>
            <Box sx={{ display: "flex", alignItems: "baseline", gap: 1, mt: 0.5 }}>
                <Typography sx={{ fontWeight: 800, color: C.text, fontSize: "1.5rem", lineHeight: 1.1 }}>{value}</Typography>
...
export function TopList({
    ...
    return (
        <Box>
            {items.map((it) => {
                const row = (
                    <Box sx={{ position: "relative", py: 0.8, px: 1, borderBottom: `1px solid ${C.border}`, "&:hover": it.href ? { bgcolor: C.panelHover } : undefined }}>
                        <Box sx={{ position: "absolute", left: 0, top: 0, bottom: 0, width: `${(it.value / max) * 100}%`, bgcolor: color, opacity: 0.12, borderRight: `2px solid ${color}` }} />
                        <Box sx={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 1 }}>
                            <Typography sx={{ color: C.text, fontSize: "0.82rem", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "68%" }}>{it.label}</Typography>
                            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                                {it.chip}
    ...
    return (
        <Box sx={{ display: "flex", alignItems: "center", gap: 2, flexWrap: "wrap" }}>
            <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
                ...
                <text x={cx} y={cy} textAnchor="middle" dominantBaseline="central" fill={C.text} fontSize="0.95rem" fontWeight="700">
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 0.5 }}>
                {data.map((d, i) => (
                    <Box key={d.label} sx={{ display: "flex", alignItems: "center", gap: 0.75 }}>
                        <Box sx={{ width: 9, height: 9, borderRadius: "2px", bgcolor: CHART[i % CHART.length] }} />
                        <Typography sx={{ fontSize: "0.76rem", color: C.textDim }}>
                            {d.label} <Box component="span" sx={{ color: C.text, fontWeight: 600 }}>{((d.value / total) * 100).toFixed(0)}%</Box>
                        </Typography>
                    </Box>
                ))}
            </Box>
...
export function StatusChip({ label, tone = "neutral" }: { label: string; tone?: keyof typeof STATUS }) {
    const s = STATUS[tone];
    return <Chip label={label} size="small" sx={{ height: 20, fontSize: "0.68rem", fontWeight: 600, color: s.color, bgcolor: s.bg, border: `1px solid ${s.color}33`, borderRadius: "6px" }} />;
}

export function SectionError({ message }: { message: string }) {
    return <Box sx={{ p: 1.5, borderRadius: "8px", bgcolor: "rgba(251,191,36,0.07)", border: "1px solid rgba(251,191,36,0.25)", color: C.warn, fontSize: "0.78rem" }}>{message}</Box>;
}
