...
export default async function TrafficPage() {
    ...
    const charts = await Promise.all(
        inv.zones.map((z) =>
            zoneTraffic(z.id).then((m) => ({ name: z.name, m })),
        ),
    );
    ...
    return (
        <Box>
            <PageHeader
                title="Traffic"
                subtitle="HTTP requests & bandwidth per zone · 24h"
            />
            {inv.errors.zones && (
                <SectionError
                    message={`Couldn't list zones: ${inv.errors.zones.error}`}
                />
            )}
            {inv.zones.length === 0 ? (
                    ...
                    {charts.map(({ name, m }) => (
                        <Panel key={name} title={name}>
                            {m.available ? (
                                ...
                                <SectionError
                                    message={`Zone analytics unavailable: ${m.error}`}
                                />
                            )}
                        </Panel>
                    ))}
