...
export default async function WorkersPage() {
    ...
    const metrics = await Promise.all(
        inv.workers.map((w) => workersMetrics(w.id).then((m) => ({ name: w.id, m }))),
    );

    return (
        <Box>
            <PageHeader title="Workers" subtitle="Invocations, errors and subrequests per script · 24h" />
            {inv.errors.workers && <SectionError message={`Couldn't list Workers: ${inv.errors.workers.error}`} />}
                        ...
                        <Panel key={name} title={autoLabel(name)}>
                            {m.available ? (
                                <MetricChart
                                    points={m.points}
                                    series={[
                                        { key: "requests", label: "Requests", color: "#fbbf24" },
                                        { key: "errors", label: "Errors", color: "#f87171" },
                                    ]}
                                />
                            ) : (
                                <SectionError message={`Metrics unavailable: ${m.error}`} />
                            )}
                        </Panel>
                    ))}
                </Box>
